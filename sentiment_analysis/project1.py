...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta_sum = theta = np.zeros(n_features)
...
def pegasos_single_step_update(feature_vector, label, L, eta, theta, theta_0):
    ...
    """
    # Your code here
    prediction = label * (np.dot(theta, feature_vector) + theta_0)
...
def classifier_accuracy(classifier, train_feature_matrix, val_feature_matrix, train_labels, val_labels, **kwargs):
    ...
    Args:
        `classifier` - A learning function that takes arguments (feature matrix, labels, **kwargs) and returns (theta, theta_0)
        `train_feature_matrix` - A numpy matrix describing the training data. Each row represents a single data point.
        `val_feature_matrix` - A numpy matrix describing the validation data. Each row represents a single data point.
...
def extract_words(text):
    ...
    Args:
        `text` - A string containing a sentence or phrase.
    ...
    for c in punctuation_digits:    text = text.replace(c, f' {c} ')
